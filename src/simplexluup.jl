...
function simplexluup(c::Vector{Float64}, A::SparseMatrixCSC{Float64, Int64}, b::Vector{Float64},
                     𝔹=Int[], L=0, U = 0, prow=collect(1:A.m), Rs=Vector{Float64}(undef, A.m),
                     xB::Vector{Float64}=Float64[]; max_iter::Int64 = 20000, maxups::Int64 = 10)

  ϵ = norm(c)*1e-13
  m, n = A.m, A.n # preparations
  iter = 0; ups = 0; maxed = false
  P = Vector{Vector{Int64}}(undef, maxups)
  MP = Vector{SparseVector{Float64, Int64}}(undef, maxups)
  Lp = Vector{Int64}(undef, m + 1)
  Up = Vector{Int64}(undef, m + 1)
  w = Array{Float64, 1}(undef, m)
  d = Vector{Float64}(undef, m)
  λ = Array{Float64, 1}(undef, m)
  Ucolp = Array{Float64, 1}(undef, m)
  tempperm = Vector{Int64}(undef, m)
  pcol = collect(1 : m)
  signb = sign.(b)
